...
namespace Elixir
{
        ...
        )
        {
            ...
            if (isScalar)
                return expression.ValueType == EMaterialValueType::Float
                    ? expression.Code : "(" + expression.Code + ").x";
    ...
    uint32_t MaterialGraph::AddNode(std::unique_ptr<IMaterialNode> node)
    {
        if (!node)
            return 0;
    ...
    void MaterialGraph::Connect(const uint32_t fromNode, const uint32_t toNode, const uint32_t toSlot)
    {
        ...
        it->second.Inputs[toSlot] = static_cast<int32_t>(fromNode);
    }
    ...
    const IMaterialNode* MaterialGraph::FindNode(const uint32_t id) const
    {
    ...
    ) const
    {
        ...
        const auto graphIt = m_Nodes.find(id);
        if (graphIt == m_Nodes.end() || visiting.contains(id))
            return { "0.0", EMaterialValueType::Float };

        visiting.insert(id);
        const auto& graphNode = graphIt->second;
        const auto& inputDefinitions = graphNode.Node->GetInputs();
        std::vector<SMaterialExpression> inputs;
        inputs.reserve(inputDefinitions.size());

        for (size_t slot = 0; slot < inputDefinitions.size(); ++slot)
        {
            const int32_t sourceId = slot < graphNode.Inputs.size() ? graphNode.Inputs[slot] : -1;
            if (sourceId >= 0)
                inputs.push_back(EmitNode(static_cast<uint32_t>(sourceId), emitted, visiting, body, bindings));
            else
                inputs.push_back({
                    inputDefinitions[slot].DefaultExpression,
                    inputDefinitions[slot].DefaultValueType
                });
        }

        const SMaterialEmitContext context(inputs, bindings);
        SMaterialExpression expression = graphNode.Node->Emit(context);
        const std::string variable = "n" + std::to_string(id);
        body += "   " + std::string(SMaterialEmitContext::TypeName(expression.ValueType)) + " " +
