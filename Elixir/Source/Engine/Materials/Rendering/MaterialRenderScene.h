...
namespace Elixir::Materials::Rendering
{
    ...
    struct SRenderGeometry
    {
        ...
        std::vector<SConstantBufferBinding> ConstantBuffers;
    ...
    public:
         ...
         */
        uint32_t AddGeometry(SRenderGeometry geometry);
         ...
         */
        const SRenderGeometry* FindGeometry(uint32_t index) const;

        /**
         * @brief Returns the material draws in insertion order.
         * @return Read-only view of the frame-local draw items.
         */
        std::span<const SRenderItem> GetItems() const { return m_Items; }

    private:
        std::vector<SRenderGeometry> m_Geometries;
