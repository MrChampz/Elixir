...
TEST(AetherSystemInstanceTest, AppliesOverridesOnlyToExposedParameters)
{
    ...
    EXPECT_EQ(instance.GetParameterRevision(), initialParameterRevision + 1);

    const auto tint = instance.ResolveParameterValue(0);
    EXPECT_FLOAT_EQ(tint.x, 0.25f);
    EXPECT_FLOAT_EQ(tint.y, 0.5f);
    EXPECT_FLOAT_EQ(tint.z, 0.75f);
    EXPECT_FLOAT_EQ(tint.w, 1.0f);

    const auto colorChunk = instance.ResolveParameterValue(1);
...
TEST(AetherSystemInstanceTest, StoresWorldTransformWithoutChangingCompiledSystem)
{
    ...
    EXPECT_FLOAT_EQ(instance.GetWorldTransform()[3].x, 5.0f);
    EXPECT_FLOAT_EQ(instance.GetWorldTransform()[3].y, 2.0f);
    EXPECT_FLOAT_EQ(instance.GetWorldTransform()[3].z, -3.0f);
    EXPECT_EQ(&instance.GetCompiledSystem(), compiledSystem.get());
}